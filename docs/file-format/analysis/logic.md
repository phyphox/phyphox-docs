...
Since version 1.4 (phyphox 1.0.6) you may set an input to be of the type *empty*. Effectively, this does not change the output as nothing is written, but can make sense in combination with setting the output to *clear*. In this case, the if module behaves in a somewhat special way as it only clears its output if a matching input is connected. For example, if you set the true input to *empty*, do not connect the false input and set the output to *clear*, it will only clear the output if the condition is true, but leave it alone otherwise. This can be used as a reset condition. Note that this clearing only empties the output buffer; it does not restore the buffer's *init* values, which only the user's clear-data action does.
