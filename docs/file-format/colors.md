...
These are the only two accepted forms: a color value that is neither a named color from the table below (names are matched case-insensitively) nor exactly six hex digits (or eight, see below) is an error, and the experiment will not load.

*Planned for file format 1.21:* a color may carry a fourth byte for its opacity, written as eight hex digits `RRGGBBAA` (again with or without the hash). `ff` is opaque and `00` fully transparent; a six-digit color is opaque, so existing files are unchanged. This applies to every color attribute in the format - a graph line, the fill of a separator, the text of a value, the stops of a color map. It matters most inside a [stack](views/groups.md#view-element-stack), where a translucent plot or tint lets an image below show through. The light-mode adjustment described below keeps the alpha byte as it is. Note the digit order: it is the CSS order, red first and alpha last, not the `AARRGGBB` order some platforms use internally. A file using it must declare version 1.21, and older apps refuse it.
