...
How the input tags form datasets: every *y* input is one dataset (one curve). When a graph provides exactly as many x inputs as y inputs (z inputs do not count), they are matched 1-on-1 in their order of appearance, whatever the interleaving — so the n-th x belongs to the n-th y. With fewer x than y inputs, each y is plotted against the most recent preceding x input — so several datasets can share one x axis by naming its data container once, before the first y — and a dataset with no preceding x at all is plotted against its element index. Any x input that no y input uses — trailing, or shadowed by a later x before any y consumed it — is an error, and every input must name its axis. Until both apps implement these decided rules (see the notices below), the portable forms are the strictly interleaved order used by the example above and the plain 1-on-1 orders that pair the same way on both apps today.
