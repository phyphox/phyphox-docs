...
Some sensors are available in two versions on a device: a *calibrated* one, where the operating system corrects the raw readings (for the bias of a gyroscope, or for hard- and soft-iron distortions of the magnetic field, for example), and an *uncalibrated* one that passes the raw readings on. Calibration usually gives the more useful numbers for a physics experiment, but it also means the values may jump when the system updates its calibration. Since file format 1.21 the *preferUncalibrated* attribute chooses which version an experiment starts with; the user can still switch to the other version from the experiment menu while the measurement is stopped, as was possible for the magnetometer before. Which sensor types come in two versions depends on the platform and device: the magnetometer does on both Android and iOS, and Android additionally exposes uncalibrated versions of the accelerometer and the gyroscope on most devices.
