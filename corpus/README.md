...
- `invalid/` — files carrying a documented defect each, nearly all
  ...
  a file came from. Seventeen were folded in from phyphox-ios'
  `phyphoxTests/incorrect-files/` on 2026-08-29, fixtures that only the iOS
  suite had ever run; the rest of that set could not follow, because a file
  here has to produce a `validate_experiments` finding and theirs do not —
  each is a rule the spec does not model yet.
